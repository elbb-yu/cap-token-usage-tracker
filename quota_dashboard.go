...
function render(data){current=Array.isArray(data.items)?data.items:[];const root=document.getElementById('content');if(!current.length){root.className='empty';root.innerHTML='还没有观察到 API Key。Key 至少成功产生一条用量记录后会自动出现，也可以点击“添加 / 设置 Key 额度”。';return}root.className='';let html='<table><thead><tr><th>API Key</th><th>标签</th><th>实际费用</th><th>最高额度</th><th>剩余额度</th><th>使用比例</th><th>价格覆盖</th><th>状态</th><th>操作</th></tr></thead><tbody>';current.forEach(item=>{const limited=!!item.limited,pct=limited&&item.limit_usd>0?Math.min(100,item.used_usd/item.limit_usd*100):0,barClass=pct>=100?'danger':pct>=80?'warn':'',missing=Number(item.unpriced_requests||0),blocked=!!item.blocked;html+='<tr><td class="key">'+esc(item.masked_key||'未识别')+'</td><td>'+esc(item.label||'—')+'</td><td class="money">'+(limited?usd(item.used_usd):'—')+'</td><td class="money">'+(limited?usd(item.limit_usd):'无限制')+'</td><td class="money">'+(limited?usd(item.remaining_usd):'—')+'</td><td>'+(limited?'<div class="bar"><div class="fill '+barClass+'" style="width:'+pct.toFixed(1)+'%"></div></div>':'—')+'</td><td>'+(limited?(missing?'<span class="status warn">缺 '+missing+' 次</span>':'<span class="status ok">完整</span>'):'—')+'</td><td><span class="status '+(blocked?'danger':limited?'ok':'warn')+'">'+esc(blocked?(item.block_reason||'已暂停'):limited?'可用':'未限额')+'</span></td><td><div class="row-actions">'+(limited?'<button data-edit="'+esc(item.id)+'">修改</button><button class="danger" data-reset="'+esc(item.id)+'">重置</button>':'<button data-new="1">设置</button>')+'</div></td></tr>'});html+='</tbody></table>';root.innerHTML=html;document.getElementById('foot').textContent='更新时间：'+new Date(data.generated_at).toLocaleString()+' · 费用基准：当前模型价格簿';}
...
async function resetQuota(id){const item=current.find(x=>x.id===id);if(!item||!confirm('确认把 '+(item.label||item.masked_key)+' 的已用额度清零？历史用量记录不会删除。'))return;await authorized(resetURL,'POST',{id});await load()}
document.getElementById('refresh').onclick=()=>{document.getElementById('refresh').disabled=true;load()};document.getElementById('add').onclick=()=>addQuota().catch(e=>error(e.message));document.getElementById('content').onclick=e=>{const edit=e.target.closest('[data-edit]'),reset=e.target.closest('[data-reset]'),fresh=e.target.closest('[data-new]');if(edit)editQuota(edit.dataset.edit).catch(x=>error(x.message));else if(reset)resetQuota(reset.dataset.reset).catch(x=>error(x.message));else if(fresh)addQuota().catch(x=>error(x.message))};load();setInterval(load,15000);
</script></body></html>`
